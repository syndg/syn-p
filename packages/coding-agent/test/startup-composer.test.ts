...
describe("Composer prepaint", () => {
	...
	it("first frame mirrors the canonical settings-schema defaults", () => {
		expect(COMPOSER_DEFAULTS).toEqual({
			...
			tuiMode: getDefault("tui.mode"),
			fullscreenScrollbar: getDefault("tui.fullscreenScrollbar"),
			fullscreenCopyOnSelect: getDefault("tui.fullscreenCopyOnSelect"),
			fullscreenExitOutput: getDefault("tui.fullscreenExitOutput"),
	...
	it("preferences feed applies quiet mode", async () => {
		...
		applyStartupComposerPreferences({
			...
			fullscreenScrollbar: config.fullscreenScrollbar,
			fullscreenCopyOnSelect: config.fullscreenCopyOnSelect,
