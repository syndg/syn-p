...
export class TUI extends Container {
	...
	stop(): void {
		...
		if (this.#resizeInPlaceActive && this.terminal.rows > 0) {
			...
			// Park absolutely before the shell handoff moves from the tracked row.
	...
	#doRender(): void {
		...
		const wantMouse: MouseTrackingState = fullscreenOverlay
			? topOverlay.options?.mouseTracking !== false
				? "full"
				: "off"
			: this.#primaryFullscreen
				? "full"
				: topOverlay === undefined && this.#inlineMouseProvider?.() === true
					? "inline"
					: "off";
	...
	#renderAltFrame(width: number, height: number): void {
		...
		this.#emitAltFrame(prepared, width, height, markers, forceClearRows);
	}
	...
	#emitAltFrame(
		...
		const hasProtocolRows =
			prepared.rows.some(row => row.isImage || isOsc66Line(row.line)) ||
			this.#altPreparedRows.some(row => row.isImage || isOsc66Line(row.line));
		const canDiff =
			this.#primaryFullscreen &&
			this.#altActive &&
			!this.#resizeAltActive &&
		...
		const scroll =
			candidateScroll && Math.abs(candidateScroll.delta) >= TUI.#MIN_ACCELERATED_SCROLL_ROWS
				? candidateScroll
				: undefined;
		let previousLines: readonly (string | undefined)[] = this.#altPreviousLines;
		let previousRows: readonly (PreparedLine | undefined)[] = this.#altPreparedRows;
		if (scroll) {
			const shiftedLines: (string | undefined)[] = [...this.#altPreviousLines];
			const shiftedRows: (PreparedLine | undefined)[] = [...this.#altPreparedRows];
			...
			for (let row = scroll.top; row <= scroll.bottom; row++) {
				...
				shiftedLines[row] =
					source >= scroll.top && source <= scroll.bottom ? this.#altPreviousLines[source] : undefined;
				shiftedRows[row] =
					source >= scroll.top && source <= scroll.bottom ? this.#altPreparedRows[source] : undefined;
			}
			previousLines = shiftedLines;
			previousRows = shiftedRows;
		}
		...
		if (canDiff) {
			for (let row = 0; row < height; row++) {
				...
				if (
					forcedRows.has(row) ||
					prepared.lines[row] !== previousLines[row] ||
					previous === undefined ||
		...
		if (canDiff && scroll === undefined && changedRows.length === 0 && cursorSame) {
